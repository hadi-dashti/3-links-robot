# Full updated training script including combined loss with potential energy 
# Mean End-Effector Position Error: 0.2322 meters
#Max End-Effector Position Error: 1.3050 meters
import pandas as pd
import numpy as np
...
# === Combined loss function with potential energy ===
@tf.function
def combined_loss_with_energy(y_true, y_pred):
    ...
    def potential_energy(thetas):
        theta1, theta2, theta3 = tf.unstack(thetas, axis=1)
        h1 = 0.5 * tf.sin(theta1)
        h2 = tf.sin(theta1) + 0.5 * tf.sin(theta1 + theta2)
        h3 = tf.sin(theta1) + tf.sin(theta1 + theta2) + 0.5 * tf.sin(theta1 + theta2 + theta3)
        U = h1 + h2 + h3
        return U

    U_pred = potential_energy(theta_pred)
    pe_loss = tf.reduce_mean(U_pred)

    total_loss = 0.6 * mse_loss + 0.3 * fk_loss + 0.1 * pe_loss
    return total_loss
...
model = build_model()
model.compile(optimizer='adam', loss=combined_loss_with_energy, metrics=['mae'])
...
# === Predict and convert outputs to angles ===
...
plt.tight_layout()
plt.savefig("picture/enhanced_loss_prediction_with_energy.png", dpi=300)
plt.show()

# === Evaluate final position error ===
end_effector_errors = []
...
plt.title('End-Effector Position Error (With Energy Loss)')
plt.xlabel('Position Error (meters)')
plt.ylabel('Number of Samples')
plt.grid(True)
plt.tight_layout()
plt.savefig("picture/enhanced_loss_error_with_energy.png", dpi=300)
plt.show()
